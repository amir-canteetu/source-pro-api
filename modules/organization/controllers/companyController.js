...
const validateCompany = [
  ...
  body("company_type")
    ...
    .isIn(["service_provider", "procuring_company"])
  ...
  body("employee_count")
    .optional()
    .isInt()
    .withMessage("Employee count must be an integer"),
  ...
  body("operating_since")
    .optional()
    .isInt()
    .withMessage("Operating since must be an integer"),
...
const createCompany = [
  ...
  async (req, res, next) => {
    ...
    try {
      ...
      res
        .status(201)
        .json({ message: "Company created successfully", companyId: insertId });
    } catch (error) {
      next(error);
    }
  },
...
const updateCompany = [
  ...
  async (req, res, next) => {
    ...
    try {
      const rowsAffected = await Company.updateCompany(
        companyId,
        updatedCompany,
      );
